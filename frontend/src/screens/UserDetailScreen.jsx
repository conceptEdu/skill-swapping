...
const UserDetailScreen = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const config = { headers: { Authorization: `Bearer ${userInfo.token}` } }; // ✅ सही किया
        const { data } = await axios.get(`/api/users/${id}`, config); // ✅ सही किया
        setUser(data);
      } catch (err) {
        console.error(err.response?.data?.message || 'User fetch failed');
      }
    };
    if (userInfo) {
      fetchUser();
    ...
    <div className="max-w-4xl mx-auto p-6 mt-10">
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="md:w-1/3 bg-blue-600 p-8 text-white flex flex-col items-center">
          ...
          <button
            onClick={() => navigate(`/chat/${user._id}`)} // ✅ सही किया
        ...
        <div className="md:w-2/3 p-8">
          ...
          <p className="text-gray-600 mb-8">
            {user.bio || 'इस यूजर ने अभी कोई बायो नहीं लिखा है।'}
          </p>

          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-green-600 uppercase">
                विशेषज्ञता (Teaching Skills)
              </h3>
              <div className="flex flex-wrap gap-2 mt-2">
                {user.teachSkills?.map((s, i) => (
                  <span
                    key={i}
                    className="bg-green-100 text-green-800 px-4 py-1 rounded-lg text-sm border border-green-200"
                  >
                    {s.skillName}
                  </span>
                ))}
            ...
            <div>
              <h3 className="text-sm font-semibold text-blue-600 uppercase">
                सीखने की इच्छा (Learning Skills)
              </h3>
              <div className="flex flex-wrap gap-2 mt-2">
                {user.learnSkills?.map((s, i) => (
                  <span
                    key={i}
                    className="bg-blue-100 text-blue-800 px-4 py-1 rounded-lg text-sm border border-blue-200"
                  >
                    {s.skillName}
                  </span>
                ))}
